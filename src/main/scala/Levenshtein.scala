...
object Levenshtein {
  ...
  def distance(s1i:String, s2i:String)={
    val s1: String = normalizeString(s1i)
    val s2: String = normalizeString(s2i)
  ...
  def normalizeString(s: String): String = {
    s.toLowerCase().map(stripLowercasePolishCharacters(_))
  }

  def stripLowercasePolishCharacters(c: Char): Char = {
    assert(!c.isUpper) // no, this is not the same as c.isLower ;)
    c match {
      case 'ą' => 'a'
      case 'ć' => 'c'
      case 'ę' => 'e'
